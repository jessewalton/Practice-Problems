...
class ChessGame(object):
    ...
    def test(self):
        ...
        while (playingGame):
            ...
            selectedPiece = self.chess_board.getPieceFromUid(_uid)
            self.chess_board.showValidMoves(selectedPiece)
...
class ChessBoard(object):
    ...
    def showValidMoves(self, piece):
        moves = self.getValidMoves(piece)

        print("Move list: %s %s (%s)" % (piece.color, piece.name, piece.uid), end=" ")
        print("at (%s, %s)" % (piece.x, piece.y))

        for index, (x_move, y_move) in enumerate(moves, 1):
            print("\t%s. (%s, %s)" % (index, x_move, y_move))
